...
int result;
	...
	}
	{
			...
			if (rtp_ssl_init_server(&sslSession, sockHandle,sslContext) >= 0)
			if (rtp_ssl_accept(sslSession) < 0)
			{
				rtp_net_closesocket(sockHandle);
				return (-1);
			}
			result = HTTPS_ServerProcessOneConnection(sslSession, server, sockHandle, &clientAddr);
	}
	return (result);
}
                                    ...
                                    HTTPSession *session)
{
	...
	{
		rtp_ssl_close_session(((RtpSslSockStruct*)session->streamPtr)->sslSession);
