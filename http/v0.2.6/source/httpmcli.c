...
#ifndef HTTP_MANAGED_CLIENT_INCLUDE_UDP
#    define HTTP_MANAGED_CLIENT_INCLUDE_UDP
#endif
...
const HTTP_UINT16 httpDefaultPort[HTTP_NUM_SESSION_TYPES] = {
		80,
		443,
		1900
	};
...
/*---------------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------------*/
/** @memo Send an HTTP request (low level).

    @doc  HTTP_ManagedClientRequestEx is provided for implementing http based protocols such as SOAP, SSDP and GENA. Use caution when calling this function. Most application needs can be satisfied
    by using one of the higher level functions \Ref{HTTP_ManagedClientGet}, \Ref{HTTP_ManagedClientPut}, or \Ref{HTTP_ManagedClientPost}.
...
int HTTP_ManagedClientRequestEx (
 		HTTPManagedClientSession* session, /** the session over which to perform the GET */
		const HTTP_CHAR* method,           /** Commonly GET, POST or PUT. Also supports SOAP and GENA methods SUBSCRIBE, UNSUBSCRIBE, M-SEARCH, M-POST etc. */
		const HTTP_CHAR* path,             /** For GET operations the file to get on the server for PUT, the file to PUT */
		RTP_TIMESTAMP* ifModifiedSince,     /** For GET operation instruct the server to send the file only
		                                       if it has been modified after this date */
		HTTP_CHAR* contentType,            /** the mime type of the data on a POST or PUT ("text/xml", for example) */
		HTTP_INT32 contentLength,          /** HTTP_CONTENT_LENGTH_NONE for GET operation. For PUT and POST the contentLength is optional. If the contentLength is not specified
		                                       then the data is sent in chunked transfer-encoding (each call to \Ref{HTTP_ManagedClientWrite} will generate a single chunk).
		                                       In either case, \Ref{HTTP_ManagedClientWriteDone} must be called once all data has been written. */
		HTTPRequest* request               /** An uninitialized HTTPRequest sized memory buffer for internal use must be provided by the caller */
	)
{
	int result = -1;
...
/*---------------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------------*/
/** @memo Send CR LF to terminate HTTP request header and update session state.  (low level).

    @doc  HTTP_ManagedClientRequestHeaderDone is provided for implementing http based protocols such as SOAP, SSDP and GENA. Use caution when calling this function. Most application needs can be satisfied
    by using one of the higher level functions \Ref{HTTP_ManagedClientGet}, \Ref{HTTP_ManagedClientPut}, or \Ref{HTTP_ManagedClientPost}.
...
{
	HTTP_UINT32* currentTimeMsec = (HTTP_UINT32*) data;
...
/*---------------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------------*/
/** @memo Retrieve the socket associated with the http session. (low level)

    @doc Retrieves the socket associated with the http session. This a low level function is made available for implementing protocols like SOAP and GENA.
    Use caution when calling this function. Most application needs can be satisfied by using one of the higher level functions \Ref{HTTP_ManagedClientRead} or \Ref{HTTP_ManagedClientWrite}.

    @return The open socket associated this session.
